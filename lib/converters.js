...
import {  novat } from "../models/novat"
...
export const xml2json = xml => {
    ...
    if (Invoice) {
        ...
        res = {
            ...
            vat: Invoice["cac:TaxTotal"]["cac:TaxSubtotal"]["cac:TaxCategory"]["cbc:ID"] === "S" ? true : false,
...
export const json2xml = obj => {
    let res;
   console.log(Boolean(obj.vat))
    obj =  Boolean(obj.vat) ? vat(obj) : novat(obj)
   
    res = create(obj)
    res.end({ prettyPrint: true })
    return res
}
